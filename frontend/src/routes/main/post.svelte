		...
		<div class="card w-100 shadow-xss rounded-xxl border-0 p-4 mb-4">
			<div class="card-body p-0 d-flex">
				<figure class="avatar me-3">
					{#if !post.image}
						<img
							src="//ui-avatars.com/api/?name={post.firstName +
								' ' +
								post.lastName}e&size=100&rounded=true&color=fff&background=random"
				...
				<p class="fw-500 text-grey-500 lh-26 font-xssss w-100 mb-2">
					{#if post?.content.length > 200}
						{#if post.display || post.display == undefined}
							...
							<a href="#"
								class="fw-600 text-primary ms-2"
								on:click={() => {
									if (post.display == undefined) {
										post.display = false;
									} else {
						...
						{:else}
							...
							<a href="#"
								...
								style="cursor:pointer;">Show Less</a
							>
						{/if}
					{:else}
						{post.content}
			...
			<div class="card-body d-block p-0 mb-3">
				<div class="row ps-2 pe-2">
					<div class="col-sm-12 p-1">
						<img src={post.image} class="rounded-3 w-100" alt="" />
					</div>
				</div>
			</div>
			<div
				class="card-body d-flex p-0"
